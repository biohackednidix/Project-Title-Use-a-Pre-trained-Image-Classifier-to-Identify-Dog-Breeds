...
def classify_images(images_dir, results_dic, model):
    ...
    for key, value in results_dic.items():
        ...
        # 3a: Run the classifier function on the image file
        ...
        # Define the pet image label
        real_label = value[0]

        # 3c: Check if the pet image label matches the classifier label
        if real_label in model_label:
            value.extend([model_label, 1])
        else:
            # 3d: If not a match, append 0 for no match
