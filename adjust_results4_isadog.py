...
def adjust_results4_isadog(results_dic, dogfile):
    ...
    with open(dogfile, "r") as infile:
        # Reads each line in the file and processes it
        for line in infile:
            # Strip newline characters from the line
            line = line.rstrip("\n")
            # Add dog name to dictionary if not already present
            if line not in dognames_dic:
                dognames_dic[line] = 1

    # Adds to results_dic whether labels are dogs or not
    # Adding value instead of key in below all code
    for value in results_dic.values():
        ...
        if value[0] in dognames_dic:
            if value[1] in dognames_dic:
                ...
                value.extend((1, 0))  # Only pet label is a dog
        else:
            if value[1] in dognames_dic:
                value.extend((0, 1))  # Only classifier label is a dog
            else:
